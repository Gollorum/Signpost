		...
		data.consume(
			langKey -> {
				addRenderableOnly(new TextDisplay(
					new TranslatableComponent(langKey),
			...
			d -> {
				...
				if(d.cannotTeleportBecause.isEmpty() && !isTooFarAway) {
					addRenderableOnly(new TextDisplay(
						new TranslatableComponent(LangKeys.confirmTeleport, Colors.wrap(d.waystoneName, Colors.highlight)),
						new Point(width / 2, height / 2 - 20),
						Rect.XAlignment.Center, Rect.YAlignment.Bottom,
						font
					));
					...
					if (!d.cost.isEmpty()) {
						addRenderableOnly(new TextDisplay(
							new TranslatableComponent(LangKeys.cost),
