...
public class PostHandler {
	...
	/**
	 * UUID = the player; StringSet = the discovered waystones;
	 */
	...
	/**
	 * UUID = the player; Pair.MyBlockPosSet = the discovered waystones;
	 * Pair.b.a = the waystones left to place; Pair.b.b = the signposts left to
	 * place;
	 */
		...
		if (ClientConfigStorage.INSTANCE.getCost() == null || ConfigHandler.isCreative(player)) {
			...
			for (ItemStack now : player.inventory.mainInventory) {
				if (now != null && now.getItem() != null
						&& now.getItem().getClass() == ClientConfigStorage.INSTANCE.getCost().getClass()) {
		...
		SPEventHandler.scheduleTask(new Runnable() {
			...
			public void run() {
				if (info == null) {
					...
					doPay(player, (int) player.posX, (int) player.posY, (int) player.posZ, info.destination.pos.x,
							info.destination.pos.y + 1, info.destination.pos.z);
					SPEventHandler.cancelTask(info.boolRun);
					if (!(player.getServerWorld().getWorldInfo().getWorldName()
							.equals(info.world.getWorldInfo().getWorldName()))) {
						player.mcServer.getPlayerList().transferEntityToWorld(player, player.dimension,
								player.getServerWorld(), info.world, new SignTeleporter(info.world));
					}
					if (!(player.dimension == info.destination.pos.dim)) {
						player.mcServer.getPlayerList().transferPlayerToDimension(player, info.destination.pos.dim,
								new SignTeleporter(info.world));
						// player.changeDimension(info.destination.pos.dim);
					}
					player.setPositionAndUpdate(info.destination.pos.x + 0.5, info.destination.pos.y + 1,
							info.destination.pos.z + 0.5);
				}
			}
		}, 1);
	}
	...
	public static void teleportMe(BaseInfo destination, final EntityPlayerMP player, int stackSize) {
		...
		if (canTeleport(player, destination)) {
			...
			if (world == null) {
				NetworkHandler.netWrap.sendTo(new ChatMessage("signpost.errorWorld", "<world>", destination.pos.world),
						player);
			} else {
				SPEventHandler.scheduleTask(awaiting.put(player.getUniqueID(),
						new TeleportInformation(destination, stackSize, world, new BoolRun() {
	...
	public static boolean updateWS(BaseInfo newWS, boolean destroyed) {
		if (destroyed) {
			if (allWaystones.remove(getWSbyName(newWS.getName()))) {
				for (Entry<UUID, Pair<MyBlockPosSet, Pair<Integer, Integer>>> now : playerKnownWaystonePositions
						.entrySet()) {
				}
				return true;
			}
	...
	public static boolean addDiscovered(UUID player, BaseInfo ws) {
		...
		if (playerKnownWaystonePositions.containsKey(player)) {
			...
			ret = ret | !(playerKnownWaystonePositions.containsKey(player)
					&& playerKnownWaystones.get(player).remove(ws.getName()));
			return ret;
		} else {
			MyBlockPosSet newSet = new MyBlockPosSet();
			...
			pair.b.b = ClientConfigStorage.INSTANCE.getMaxSignposts();
			playerKnownWaystonePositions.put(player, pair);
			return !(playerKnownWaystonePositions.containsKey(player)
					&& playerKnownWaystones.get(player).remove(ws.getName()));
		}
	}
	...
	public static boolean canTeleport(EntityPlayerMP player, BaseInfo target) {
		if (doesPlayerKnowWaystone(player, target)) {
			...
			NetworkHandler.netWrap.sendTo(new ChatMessage("signpost.notDiscovered", "<Waystone>", target.getName()),
					player);
		}
		return false;
	}
	...
	public static WorldServer getWorldByName(String world, int dim) {
		...
		forLoop: for (WorldServer now : FMLCommonHandler.instance().getMinecraftServerInstance().worldServers) {
	...
	public static boolean isHandEmpty(EntityPlayer player) {
		return player.getHeldItemMainhand() == null || player.getHeldItemMainhand().getItem() == null
				|| player.getHeldItemMainhand().getItem().equals(Item.getItemFromBlock(Blocks.AIR));
	}
