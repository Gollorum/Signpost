...
public class ModelTypeRegistry {
     ...
     * {@link ResourceLocation}'s own codec would read that as {@code minecraft:spruce}, so this one completes a
    ...
    public static final StreamCodec<ByteBuf, ResourceKey<PostBlock.ModelType>> KEY_STREAM_CODEC =
        ByteBufCodecs.STRING_UTF8.map(
            name -> ResourceKey.create(REGISTRY_KEY, ResourceLocation.parse(name)),
            key -> key.location().toString()
        );
