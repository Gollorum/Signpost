...
public class BlockEventListener {
    ...
    public static boolean onBeforeBlockPlace(BlockItem item, BlockPlaceContext context) {
        if(item.getBlock() instanceof WithCountRestriction) {
            BlockRestrictions.Type restrictionType = ((WithCountRestriction)item.getBlock()).getBlockRestrictionType();
            PlayerHandle player = PlayerHandle.from(context.getPlayer());
            return BlockRestrictions.getInstance().tryDecrementRemaining(restrictionType, player);
        }
        return true;
    }
