...
public class ModelWaystoneImpl extends ModelWaystone {
    ...
    protected @NotNull MapCodec<? extends BaseEntityBlock> codec() {
        ...
        ).apply(builder, (properties, variantName) -> new ModelWaystoneImpl(
            ModelWaystone.variants.stream().filter(v -> Objects.equals(v.name, variantName)).findAny().orElseThrow(),
            properties
        )));
    }
}
